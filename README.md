...
| `mizchi/x/fs/watch` | Filesystem watcher with auto-dispatch backend (FSEvents on macOS, polling fallback elsewhere). `start`, `Watcher::next`, `Watcher::close`, `start_polling`, `Event`, `EventKind` |
